...
import { Loader2, Monitor, MoveDown, MoveUp, RefreshCw, Smartphone, Sparkles, Tablet, WandSparkles } from "lucide-react";
...
export default function WebsiteBuilder({ businessId }: { businessId: number }) {
  ...
  const generateDraft = trpc.website.generateDraft.useMutation({ onSuccess: result => { setSections(result.draft.sections.map((section, displayOrder) => ({ sectionType: section.sectionType, displayOrder, enabled: true, config: section.config }))); setSeoTitle(result.draft.seoTitle); setMetaDescription(result.draft.metaDescription); setDesign(null); setSelectedIndex(0); setAiGenerated(true); setDirty(true); } });
  const regenerateSection = trpc.website.regenerateSection.useMutation({ onSuccess: result => { setSections(currentSections.map((section, index) => index === selectedIndex ? { ...section, config: result.config } : section)); setSectionInstruction(""); setDirty(true); } });
  ...
  useEffect(() => { if (!builder.data) return; setSeoTitle(builder.data.page.seoTitle ?? ""); setMetaDescription(builder.data.page.metaDescription ?? ""); }, [builder.data?.page.seoTitle, builder.data?.page.metaDescription]);
  ...
  return <div className="space-y-6">
    ...
    {aiGenerated && <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950"><p className="font-semibold">Factual AI website draft ready</p><p className="mt-1 text-emerald-800">Review the generated copy, edit any section, then save the draft. The AI used only owner-scoped business facts and did not create ratings, reviews, or claims.</p></div>}
    ...
    <div className="grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)_260px]">
      ...
      <main className="min-w-0 rounded-2xl border border-slate-200 bg-slate-100 p-3 sm:p-5"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-semibold">Live canvas</h3><p className="text-xs text-slate-500">{dirty ? "Unsaved changes" : "Saved design"}</p></div><div className="flex rounded-xl bg-white p-1 shadow-sm" role="group" aria-label="Website preview mode">{([['desktop', Monitor, 'Desktop'], ['tablet', Tablet, 'Tablet'], ['mobile', Smartphone, 'Mobile']] as const).map(([value, Icon, label]) => <Button key={value} variant={mode === value ? "default" : "ghost"} size="sm" className="gap-1 rounded-lg px-2" aria-label={`Show ${value} preview`} aria-pressed={mode === value} onClick={() => setMode(value)}><Icon className="size-4" /><span>{label}</span></Button>)}</div></div><div data-preview-mode={mode} className={`mx-auto overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl transition-all ${modeWidth[mode]}`}>{detail.data ? <WebsiteRenderer preview data={{ page: { ...builder.data.page, id: builder.data.page.id ?? 0 }, business: detail.data.business, sections: previewSections.map((section, index) => ({ ...section, id: section.id ?? index + 1 })), services: detail.data.services.map(service => ({ id: service.id, name: service.name, description: service.description })), images: detail.data.images, reviews: [], category: undefined, city: undefined }} /> : <div className="p-8 text-sm text-slate-500">Loading verified listing facts…</div>}</div></main>
      <aside className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4"><div><h3 className="font-semibold">Page structure</h3><p className="mt-1 text-xs leading-5 text-slate-500">Reorder, disable, or restore sections. No business fact is generated here.</p></div><div className="space-y-2">{currentSections.map((section, index) => <div key={`${section.sectionType}-${index}`} className={`flex items-center gap-2 rounded-xl border p-2 ${selectedIndex === index ? "border-blue-400 bg-blue-50" : "border-slate-200"} ${section.enabled ? "" : "border-dashed opacity-60"}`}><button type="button" className="min-w-0 flex-1 truncate text-left text-sm font-medium" onClick={() => setSelectedIndex(index)}>{section.sectionType}</button><Button variant="ghost" size="sm" className="h-7 px-2 text-[11px]" onClick={() => toggle(index)}>{section.enabled ? "Hide" : "Show"}</Button><Button variant="ghost" size="icon" className="size-7" disabled={index === 0} onClick={() => move(index, -1)} aria-label="Move section up"><MoveUp className="size-4" /></Button><Button variant="ghost" size="icon" className="size-7" disabled={index === currentSections.length - 1} onClick={() => move(index, 1)} aria-label="Move section down"><MoveDown className="size-4" /></Button></div>)}</div><div className="border-t pt-4"><h3 className="font-semibold">Properties</h3><label className="mt-3 block text-xs font-medium text-slate-600">Theme<select className="field mt-1" value={String(currentDesign.theme ?? "modern")} onChange={event => { setDesign({ ...currentDesign, theme: event.target.value }); setDirty(true); }}><option value="modern">Modern</option><option value="editorial">Editorial</option><option value="minimal">Minimal</option></select></label><label className="mt-3 block text-xs font-medium text-slate-600">SEO title<input className="field mt-1" maxLength={180} value={seoTitle} placeholder="A clear title for search results" onChange={event => { setSeoTitle(event.target.value); setDirty(true); }} /></label><label className="mt-3 block text-xs font-medium text-slate-600">Meta description<textarea className="field mt-1 min-h-20" maxLength={300} value={metaDescription} placeholder="A factual summary of this business website" onChange={event => { setMetaDescription(event.target.value); setDirty(true); }} /></label><label className="mt-3 block text-xs font-medium text-slate-600">Corner radius<select className="field mt-1" value={String(currentDesign.radius ?? "lg")} onChange={event => { setDesign({ ...currentDesign, radius: event.target.value }); setDirty(true); }}><option value="sm">Compact</option><option value="lg">Soft</option><option value="xl">Rounded</option></select></label>{selected && <><label className="mt-3 block text-xs font-medium text-slate-600">Section label<input className="field mt-1" value={String(selected.config?.label ?? "")} placeholder={selected.sectionType} onChange={event => updateSelectedConfig("label", event.target.value)} /></label><label className="mt-3 block text-xs font-medium text-slate-600">AI refinement request<input className="field mt-1" value={sectionInstruction} placeholder="Make this clearer and shorter" onChange={event => setSectionInstruction(event.target.value)} /></label><Button className="mt-3 w-full gap-2 rounded-xl" variant="outline" disabled={regenerateSection.isPending} onClick={() => regenerateSection.mutate({ businessId, sectionType: selected.sectionType, currentConfig: selected.config ?? {}, instruction: sectionInstruction.trim() || undefined })}><RefreshCw className={`size-4 ${regenerateSection.isPending ? "animate-spin" : ""}`} />{regenerateSection.isPending ? "Refining…" : "Refine section with AI"}</Button></>}</div></aside>
    </div>
  </div>;
}
