...
export default function Home() {
  ...
  return <PageFrame>
    ...
    <section className="border-y border-[var(--jf-border)] bg-white"><div className="container jf-section"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><SectionHeading eyebrow="Published profiles" title="Discover local businesses" description="Browse the current profiles available on Just Finds." /><div className="flex items-center gap-2"><LocationPill /><Link href="/search" className="inline-flex items-center gap-1 text-sm font-bold text-[var(--jf-primary)]">Explore all <ChevronRight className="size-4" /></Link></div></div><div className="mt-7 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{isLoading ? [...Array(3)].map((_, index) => <div key={index} className="h-64 animate-pulse rounded-[22px] bg-slate-100" />) : cardItems.length ? cardItems.map(item => <BusinessCard item={item} key={item.id} />) : <div className="jf-empty col-span-full text-sm text-[var(--jf-muted)]">No local profiles are available for this view yet. Try a wider search.</div>}</div></div></section>

    <section className="jf-section"><div className="container grid gap-5 lg:grid-cols-[1.2fr_.8fr]"><div className="jf-card rounded-[22px] p-6 sm:p-8"><div className="flex size-11 items-center justify-center rounded-xl bg-blue-50 text-[var(--jf-primary)]"><Sparkles className="size-5" /></div><h2 className="mt-5 text-2xl font-bold tracking-[-.04em] text-[var(--jf-text)] sm:text-3xl">Tell Just Finds what you need.</h2><p className="mt-3 max-w-xl text-sm leading-7 text-[var(--jf-muted)]">Search in plain language to find a business, service, place, or job opportunity that matches your local need.</p><Link href="/search" className="jf-action-primary mt-6 inline-flex items-center gap-2 px-4 py-3 text-sm font-semibold">Try local search <ArrowRight className="size-4" /></Link></div><div className="jf-card rounded-[22px] p-6 sm:p-8"><p className="text-xs font-bold uppercase tracking-[.12em] text-[var(--jf-primary)]">Search with confidence</p><div className="mt-5 grid gap-4"><ConfidenceItem icon={BadgeCheck} title="Business information" text="Clear categories, contact details, hours, and service information when a profile provides them." /><ConfidenceItem icon={ShieldCheck} title="Owner-managed profiles" text="Business owners can keep their information up to date through a dedicated workspace." /></div></div></div></section>

    <section className="pb-14 sm:pb-20"><div className="container"><div className="jf-card rounded-[22px] p-6 sm:flex sm:items-center sm:justify-between sm:p-8"><div><p className="text-xs font-bold uppercase tracking-[.12em] text-[var(--jf-primary)]">For business owners</p><h2 className="mt-2 text-2xl font-bold tracking-[-.04em] text-[var(--jf-text)]">Own a business? Be easier to find.</h2><p className="mt-2 max-w-xl text-sm leading-6 text-[var(--jf-muted)]">Create and manage a factual local profile, respond to leads, and organize requests from one workspace.</p></div><div className="mt-5 flex flex-wrap gap-3 sm:mt-0"><Link href="/business"><Button className="jf-action-primary">Add your business</Button></Link><Link href="/business"><Button variant="outline" className="border-blue-200 text-[var(--jf-primary)] hover:bg-blue-50">Manage listing</Button></Link></div></div></div></section>
  </PageFrame>;
}
