...
export function AppointmentCalendarManager({ businessId }: { businessId: number }) {
  ...
  const save = trpc.business.saveAppointmentSettings.useMutation({ onSuccess: invalidate });
  const addBlackout = trpc.business.addAppointmentBlackout.useMutation({ onSuccess: () => { setBlackoutDate(""); setBlackoutReason(""); invalidate(); } });
  const removeBlackout = trpc.business.removeAppointmentBlackout.useMutation({ onSuccess: invalidate });
  const decide = trpc.business.decideAppointmentRequest.useMutation({ onSuccess: invalidate });
  ...
  return <div className="mt-8 space-y-5">
    ...
    <section className="rounded-2xl border border-slate-200 p-5"><div className="flex items-center gap-2"><ShieldCheck className="size-5 text-[#173d9c]" /><h2 className="font-semibold">Booking decisions</h2></div><p className="mt-1 text-sm text-slate-500">Every decision updates the related Lead CRM status and creates a private appointment history event.</p><div className="mt-4 space-y-3">{query.data?.requests.length ? query.data.requests.map(({ request, lead }) => { const draft = draftFor(request.id); const actionable = ["requested", "reschedule_requested", "proposed"].includes(request.status); return <article className="rounded-xl bg-slate-50 p-4" key={request.id}><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-semibold text-slate-900">{lead.name}</p><p className="mt-1 text-sm text-slate-600">{formatTime(request.startsAt, request.timeZone)} · {request.timeZone}</p><p className="mt-1 text-xs text-slate-500">{lead.email || lead.phone || "No contact details"} · <span className="font-semibold capitalize">{request.status.replaceAll("_", " ")}</span></p></div>{request.proposedStartsAt && <p className="rounded-lg bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-900">Proposed: {formatTime(request.proposedStartsAt, request.timeZone)}</p>}</div>{lead.message && <p className="mt-3 text-sm leading-6 text-slate-600">{lead.message}</p>}<textarea className="field mt-3 min-h-20" placeholder="Private decision note" value={draft.ownerNote} onChange={event => patchDraft(request.id, { ownerNote: event.target.value })} />{actionable && <div className="mt-3 flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-3"><div className="flex flex-wrap gap-2"><button disabled={decide.isPending || request.status === "proposed"} onClick={() => decide.mutate({ businessId, requestId: request.id, action: "approve", ownerNote: draft.ownerNote || undefined })} className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50"><CalendarCheck2 className="size-4" />Approve</button><button disabled={decide.isPending} onClick={() => decide.mutate({ businessId, requestId: request.id, action: "reject", ownerNote: draft.ownerNote || undefined })} className="inline-flex items-center gap-1.5 rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-sm font-semibold text-rose-700 disabled:opacity-50"><XCircle className="size-4" />Reject</button></div>{request.status !== "proposed" && <div className="flex flex-col gap-2 sm:flex-row"><select aria-label={`Propose a new time for ${lead.name}`} className="field flex-1" value={draft.proposedStartsAt} onChange={event => patchDraft(request.id, { proposedStartsAt: event.target.value })}>{slots.length ? slots.map(slot => <option key={new Date(slot.startAt).toISOString()} value={new Date(slot.startAt).toISOString()}>{formatTime(slot.startAt, request.timeZone)}</option>) : <option value="">No alternative slots available</option>}</select><button disabled={decide.isPending || !draft.proposedStartsAt} onClick={() => decide.mutate({ businessId, requestId: request.id, action: "propose_time", startsAt: draft.proposedStartsAt, ownerNote: draft.ownerNote || undefined })} className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-semibold text-[#173d9c] disabled:opacity-50">Propose time</button></div>}</div>}{request.status === "proposed" && <p className="mt-3 rounded-xl bg-amber-50 p-3 text-sm text-amber-900">Waiting for the customer to accept the proposed time from their secure appointment page.</p>}</article>; }) : <p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-500">No booking requests yet. New requests will appear here and in Lead CRM.</p>}</div></section>
