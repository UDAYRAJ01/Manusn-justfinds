...
export function PageFrame({ children, className }: { children: React.ReactNode; className?: string }) {
  ...
  return <div className={cn("jf-page flex min-h-screen flex-col text-[var(--jf-text)]", className)}>
    ...
    <header className="sticky top-0 z-40 border-b border-[var(--jf-border)] bg-white/95 backdrop-blur-xl">
      <div className="container flex h-[72px] items-center justify-between gap-3">
        <JustFindsLogo />
        <Link href="/search" className="hidden items-center gap-2 rounded-[10px] border border-[var(--jf-border)] bg-white px-3 py-2 text-xs font-semibold text-[var(--jf-muted)] shadow-sm lg:inline-flex"><MapPin className="size-3.5 text-[var(--jf-primary)]" />Search by city<ChevronDown className="size-3.5 text-slate-400" /></Link>
        <nav className="hidden flex-1 items-center justify-center gap-1 md:flex" aria-label="Main navigation">
          {links.map(link => <Link key={link.href} href={link.href} className={cn("rounded-[10px] px-3.5 py-2 text-[13px] font-semibold transition-colors", location.startsWith(link.href) ? "bg-blue-50 text-[var(--jf-primary)]" : "text-slate-700 hover:bg-slate-50 hover:text-[var(--jf-primary)]")}>{link.label}</Link>)}
        </nav>
        <div className="hidden items-center gap-2 md:flex">
          ...
          <Button asChild size="sm" className="px-4 text-[13px]"><Link href={primaryHref}>{primaryLabel}</Link></Button>
        </div>
      ...
      {menuOpen && <div className="border-t border-[var(--jf-border)] bg-white px-4 py-3 md:hidden"><div className="container grid gap-1 px-0">
        ...
        <Link href={primaryHref} onClick={() => setMenuOpen(false)} className="mt-2 flex items-center justify-center rounded-[10px] bg-[var(--jf-primary)] px-4 py-3 text-sm font-semibold text-white hover:bg-[var(--jf-primary-hover)]">{primaryLabel}</Link>
      </div></div>}
    </header>
    <main id="main-content" tabIndex={-1} className="flex-1">{children}</main>
    <nav className="fixed inset-x-0 bottom-0 z-50 mx-auto flex max-w-[560px] items-center justify-around rounded-t-2xl border-x border-t border-[var(--jf-border)] bg-white/95 px-2 pb-[max(.55rem,env(safe-area-inset-bottom))] pt-2 shadow-[0_-10px_28px_rgba(15,23,42,.10)] backdrop-blur-xl md:hidden" aria-label="Mobile navigation">
    ...
    <footer className="border-t border-[var(--jf-border)] bg-white pb-24 pt-10 text-[var(--jf-text)] md:pb-10">
      <div className="container grid gap-8 md:grid-cols-[1.2fr_.8fr_.8fr_.9fr]">
        ...
        <div><p className="text-xs font-bold uppercase tracking-[.12em] text-[var(--jf-muted)]">Ready to be found?</p><p className="mt-3 text-sm leading-6 text-[var(--jf-muted)]">Create a factual business profile and manage it from your workspace.</p><Link href="/business" className="mt-4 inline-flex rounded-[10px] bg-[var(--jf-primary)] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[var(--jf-primary-hover)]">Add your business</Link></div>
      </div>
...
function MobileNav({ href, icon: Icon, label, active }: { href: string; icon: React.ElementType; label: string; active: boolean }) {
  return <Link href={href} className={cn("flex min-w-[58px] flex-col items-center gap-1 rounded-[10px] px-2 py-1.5 text-[10px] font-bold transition-colors", active ? "bg-blue-50 text-[var(--jf-primary)]" : "text-[var(--jf-muted)] hover:bg-slate-50 hover:text-[var(--jf-text)]")}><span className={cn("grid size-7 place-items-center rounded-lg", active && "bg-[var(--jf-primary)] text-white shadow-sm")}><Icon className="size-[17px]" /></span>{label}</Link>;
}
