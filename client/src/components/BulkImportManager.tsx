  ...
  return <section className="mt-8 grid gap-5 xl:grid-cols-[1.05fr_.95fr]">
    <div className="space-y-5">
      ...
      <div className="rounded-[24px] border border-indigo-100 bg-indigo-50/30 p-6 shadow-sm">
        ...
        <label className="mt-5 flex cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-indigo-200 bg-white/80 px-4 py-7 text-center transition hover:border-indigo-500"><FileSpreadsheet className="size-7 text-indigo-600" /><span className="mt-3 text-sm font-semibold text-slate-700">{highVolumeFile?.name || "Choose a large CSV or Excel file"}</span><span className="mt-1 text-xs text-slate-500">Maximum 100,000 rows and {HIGH_VOLUME_FILE_LIMIT_LABEL}</span><input className="sr-only" type="file" accept=".csv,.xlsx,.xls" disabled={beginHighVolume.isPending || queueHighVolume.isPending || Boolean(highVolumeUploadProgress)} onChange={event => void stageHighVolumeFile(event.target.files?.[0])} /></label>
        {highVolumeUploadProgress && <p className="mt-3 flex items-center gap-2 text-xs text-indigo-700"><LoaderCircle className="size-4 animate-spin" />Securely uploading part {highVolumeUploadProgress.uploadedParts} of {highVolumeUploadProgress.totalParts}. Keep this page open until upload completes.</p>}
