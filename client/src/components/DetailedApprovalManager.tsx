...
function recordTime(value: Date | string | number | null | undefined) {
  if (!value) return "Not recorded";
  const date = new Date(value);
  return Number.isNaN(date.getTime()) ? "Not recorded" : date.toLocaleString();
}

type PublicDecision = { businessId: number; businessName: string; decision: "published" | "rejected" };
...
export function DetailedApprovalManager() {
  ...
  const confirmPublicDecision = async () => {
    if (!publicDecision) return;
    setError(null);
    try {
      await review.mutateAsync({ businessId: publicDecision.businessId, decision: publicDecision.decision, ...(publicDecision.decision === "rejected" ? { reviewerNote: "Please review your profile details before resubmitting." } : {}) });
      setNotice(publicDecision.decision === "published" ? "Listing approval recorded. The listing can now be published through the authorised workflow." : "Return decision recorded. The owner can correct the factual profile and resubmit it.");
    } catch (reason) { setError(reason instanceof Error ? reason.message : "The listing decision could not be recorded."); }
    finally { setPublicDecision(null); }
  };
  ...
  return <section className="mt-8 space-y-5">
    ...
    {isLoading ? <div className="h-64 animate-pulse rounded-[24px] bg-slate-100" /> : pending.length ? <div className="overflow-hidden rounded-[24px] border border-slate-200 bg-white">{pending.map(business => {
      ...
      {isOpen && <div className="border-t border-slate-200 bg-slate-50 p-4 sm:p-5"><div className="flex flex-wrap gap-2"><Button disabled={busy} onClick={() => setPublicDecision({ businessId: business.id, businessName: business.businessName, decision: "published" })} className="h-8 rounded-lg text-xs">Approve listing</Button><Button disabled={busy} onClick={() => setPublicDecision({ businessId: business.id, businessName: business.businessName, decision: "rejected" })} variant="outline" className="h-8 rounded-lg bg-white text-xs">Return</Button>{business.aiProfile?.id && business.aiProfile.status !== "published" && <Button disabled={busy} onClick={() => void apply(business.aiProfile!.id)} className="h-8 rounded-lg bg-emerald-700 text-xs hover:bg-emerald-800"><CheckCircle2 className="mr-1 size-3" />Apply AI profile</Button>}{business.aiProfile?.status === "published" && <Button disabled={busy} onClick={() => void revert(business.id)} variant="outline" className="h-8 rounded-lg border-amber-200 bg-amber-50 text-xs text-amber-800 hover:bg-amber-100"><RotateCcw className="mr-1 size-3" />Revert to original</Button>}</div>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Fact label="Business Name" value={business.businessName} /><Fact label="Main Category" value={business.mainCategory} /><Fact label="Subcategory" value={business.subcategory} /><Fact label="Business Type" value={business.businessType} /><Fact label="Description (About)" value={business.description} wide /><Fact label="Address" value={business.address} wide /><Fact label="City" value={business.city} /><Fact label="Locality" value={business.locality} /><Fact label="State" value={business.state} /><Fact label="Country" value={business.country} /><Fact label="Latitude" value={business.latitude} /><Fact label="Longitude" value={business.longitude} /><Fact label="Phone" value={business.phone} /><Fact label="Email" value={business.email} /><Fact label="Website" value={business.website} wide /><Fact label="Hours" value={hoursText(business.hours)} wide /><Fact label="Rating (audit only)" value={business.ratingAudit ?? "Not linked to this imported record"} /><Fact label="Total Reviews (audit only)" value={business.totalReviewsAudit ?? "Not linked to this imported record"} /></div><section aria-label="Read-only decision record" className="mt-3 rounded-xl border border-slate-200 bg-white p-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-[10px] font-semibold uppercase tracking-[.12em] text-slate-500">Decision record</p><span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-semibold text-slate-700">Read-only audit snapshot</span></div><dl className="mt-3 grid gap-2 text-xs text-slate-700 sm:grid-cols-3"><div><dt className="text-slate-500">Submitted</dt><dd className="mt-1 font-semibold">{recordTime(business.createdAt)}</dd></div><div><dt className="text-slate-500">Current listing state</dt><dd className="mt-1 font-semibold capitalize">{business.status.replace(/_/g, " ")}</dd></div><div><dt className="text-slate-500">AI generation record</dt><dd className="mt-1 font-semibold">{business.aiRewriteJob ? `${statusLabel(business.aiRewriteJob.status)} · ${recordTime(business.aiRewriteJob.createdAt)}` : "No generation record"}</dd></div></dl><p className="mt-3 text-[11px] leading-5 text-slate-500">No moderator decision has been recorded for this submitted listing. Decisions are appended by the authorised server workflow and cannot be edited here.</p></section>
    ...
    <AlertDialog open={Boolean(publicDecision)} onOpenChange={open => { if (!open) setPublicDecision(null); }}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{publicDecision?.decision === "published" ? "Approve this listing?" : "Return this listing for changes?"}</AlertDialogTitle><AlertDialogDescription>{publicDecision?.decision === "published" ? `This records an approval decision for ${publicDecision.businessName}. Confirm only after reviewing the supplied evidence and factual fields.` : `This records a return decision for ${publicDecision?.businessName}. The owner will need to correct the profile before submitting it again.`}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={busy}>Cancel</AlertDialogCancel><AlertDialogAction disabled={busy} onClick={() => void confirmPublicDecision()} className={publicDecision?.decision === "published" ? "bg-slate-950 hover:bg-slate-800" : "bg-amber-700 hover:bg-amber-800"}>{publicDecision?.decision === "published" ? "Record approval" : "Record return"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
  </section>;
}
