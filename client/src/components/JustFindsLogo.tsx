...
export function JustFindsLogo({ dark = false, variant }: { dark?: boolean; variant?: "light" | "default" }) {
  const isDark = dark || variant === "light";
  return (
    <Link href="/" className={`group inline-flex items-center gap-2.5 font-semibold tracking-[-0.045em] ${isDark ? "text-white" : "text-slate-950"}`}>
