...
type WebsiteData = {
  ...
  sections: Array<{ id: number; sectionType: string; enabled?: boolean; config?: unknown }>;
...
export default function WebsiteRenderer({ data, preview = false, onTrack }: { data: WebsiteData; preview?: boolean; onTrack?: (eventType: TrackingEvent) => void }) {
  ...
  return <main className="min-h-full bg-slate-50 text-slate-900">
    ...
    <div className="mx-auto max-w-6xl space-y-8 px-5 py-8 sm:px-8 sm:py-12">
      ...
      {data.sections.filter(section => section.enabled !== false).map(section => { const rawConfig = section.config; const copy = rawConfig && typeof rawConfig === "object" && !Array.isArray(rawConfig) ? rawConfig as { label?: string; eyebrow?: string; headline?: string; body?: string; ctaLabel?: string; ctaHref?: string; bullets?: string[]; featuredImageId?: number } : {}; const title = copy.label || copy.headline || section.sectionType; const featured = typeof copy.featuredImageId === "number" ? data.images.find(image => image.id === copy.featuredImageId) : undefined; return <section key={section.id} id={section.sectionType} className="rounded-3xl bg-white p-6 shadow-sm sm:p-8">
        {featured && section.sectionType !== "gallery" ? <img src={featured.url} alt={featured.alt || `${data.business.name} photo`} className="mb-5 aspect-[16/9] w-full rounded-2xl object-cover" /> : null}
        {section.sectionType === "hero" ? <><p className="text-sm font-semibold text-blue-700">{copy.eyebrow || "Official business information"}</p><h2 className="mt-2 text-2xl font-semibold">{copy.headline || `Welcome to ${data.business.name}`}</h2>{copy.body && <p className="mt-3 max-w-3xl leading-7 text-slate-600">{copy.body}</p>}</>
          : section.sectionType === "about" ? <><h2 className="text-2xl font-semibold">{title === "about" ? "About" : title}</h2><p className="mt-3 max-w-3xl leading-7 text-slate-600">{copy.body || data.business.aboutDescription || data.business.shortDescription || "Business information is being prepared."}</p>{copy.bullets?.length ? <ul className="mt-4 list-disc space-y-2 pl-5 text-sm text-slate-600">{copy.bullets.map((bullet, index) => <li key={`${section.id}-bullet-${index}`}>{bullet}</li>)}</ul> : null}</>
          : ["services", "menu", "rooms", "doctors", "facilities", "offers"].includes(section.sectionType) ? <><h2 className="text-2xl font-semibold capitalize">{title}</h2>{copy.body && <p className="mt-3 text-slate-600">{copy.body}</p>}{data.services.length ? <div className="mt-5 grid gap-3 sm:grid-cols-2">{data.services.map(service => <div key={service.id} className="rounded-2xl border border-slate-200 p-4"><h3 className="font-semibold">{service.name}</h3>{service.description && <p className="mt-2 text-sm leading-6 text-slate-600">{service.description}</p>}</div>)}</div> : <p className="mt-3 text-slate-500">No current offerings have been added.</p>}</>
          : section.sectionType === "gallery" ? <><h2 className="text-2xl font-semibold">{title === "gallery" ? "Gallery" : title}</h2>{copy.body && <p className="mt-3 text-slate-600">{copy.body}</p>}{data.images.length ? <div className="mt-5 grid gap-3 sm:grid-cols-3">{data.images.map(image => <img key={image.id} src={image.url} alt={image.alt || `${data.business.name} photo`} className="aspect-[4/3] w-full rounded-2xl object-cover" />)}</div> : <p className="mt-3 text-slate-500">No photos have been added.</p>}</>
          : section.sectionType === "reviews" ? <><h2 className="text-2xl font-semibold">{title === "reviews" ? "Reviews" : title}</h2>{copy.body && <p className="mt-3 text-slate-600">{copy.body}</p>}{data.reviews.length ? <div className="mt-5 grid gap-3 sm:grid-cols-2">{data.reviews.map(review => <article key={review.id} className="rounded-2xl border border-slate-200 p-4"><p className="text-sm leading-6 text-slate-600">{review.content || "Published review content is unavailable."}</p></article>)}</div> : <p className="mt-3 text-slate-500">No published reviews are available.</p>}</>
          : ["contact", "map", "cta"].includes(section.sectionType) ? <>{copy.body && <p className="mb-4 text-slate-600">{copy.body}</p>}{renderContact()}</>
          : <><h2 className="text-2xl font-semibold capitalize">{title}</h2><p className="mt-3 text-slate-600">{copy.body || "Current business information is available from the listing owner."}</p></>}
      </section>; })}
