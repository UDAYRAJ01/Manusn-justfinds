...
export const jobTypeSchema = z.enum(["full_time", "part_time", "contract", "internship", "freelance"]);

export const jobsRouter = router({
  list: publicProcedure.input(z.object({ query: z.string().max(160).default(""), city: z.string().max(140).optional(), jobType: jobTypeSchema.optional() })).query(async ({ input }) => {
    const rows = await getPublishedJobs(input.query, input.city, input.jobType);
    return rows.map(row => ({
      id: row.job.id,
      title: row.job.title,
      ...
      experience: row.job.experience ?? null,
      salary: row.job.salary ?? null,
      jobType: row.job.jobType,
      posted: row.job.publishedAt,
    }));
  }),
