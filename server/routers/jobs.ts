...
export const jobsRouter = router({
  list: publicProcedure.input(z.object({ query: z.string().max(160).default(""), city: z.string().max(140).optional() })).query(async ({ input }) => {
    ...
    if (rows.length) return rows.map(row => ({ id: row.job.id, title: row.job.title, company: row.company ?? "Independent employer", city: row.city ?? "Location flexible", category: row.job.category, experience: row.job.experience ?? "Not specified", salary: row.job.salary ?? "Not specified", jobType: row.job.jobType.replace("_", " "), posted: row.job.publishedAt }));
    const term = input.query.toLowerCase();
    return demoJobs.filter(job => !term || `${job.title} ${job.category} ${job.company}`.toLowerCase().includes(term));
  }),
